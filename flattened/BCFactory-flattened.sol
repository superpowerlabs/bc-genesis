...
contract BCOracleToken is BCNFT {
  ...
  function mint(address to, IAttributes.Rarity rarity) external virtual onlyFactory returns (uint256) {
    ...
    if (rarity == IAttributes.Rarity.COMMON) {
      if (minted.common == 300) revert CannotMint();
      minted.common++;
    } else if (rarity == IAttributes.Rarity.UNCOMMON) {
      if (minted.uncommon == 250) revert CannotMint();
      minted.uncommon++;
    } else if (rarity == IAttributes.Rarity.RARE) {
      if (minted.rare == 200) revert CannotMint();
      minted.rare++;
    } else if (rarity == IAttributes.Rarity.EPIC) {
      if (minted.epic == 150) revert CannotMint();
      minted.epic++;
    } else if (rarity == IAttributes.Rarity.LEGENDARY) {
      if (minted.legendary == 100) revert CannotMint();
      minted.legendary++;
    }
...
contract BCFactory is OwnableUpgradeable, UUPSUpgradeable {
  ...
  function _part(uint256 id) internal pure returns (uint256) {
    return (id % 4)**2;
  }
  ...
  function _validateQuadruples(
    ...
    uint256 rarity_ = rarityByIndex(id1);
    if (rarity_ ^ rarityByIndex(id2) ^ rarityByIndex(id3) ^ rarityByIndex(id4) != 0) {
  ...
  function decode(uint256 encoded, uint256 index) public pure returns (uint256) {
